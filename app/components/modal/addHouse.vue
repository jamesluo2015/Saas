...
<modal :show.sync="show" effect="fade" :backdrop="false" width="560px" :title="model.Id?'编辑库房':'添加库房'">
    <div slot="modal-body" class="modal-body ">
        ...
        <div class="col-md-12 pd_l0 clearfix select_dropdown mg_t15">
            <div class="pull-left">
                <label class="control-label pull-left f12 lineH20" for="input01"><em class="col_fb2727 mg_r5">*</em>库房名称：</label>
                <input placeholder="" v-model="model.HouseName" class="add_input w170 pull-left form-control" type="text">
            </div>
        </div>
        <div class="col-md-12 pd_l0 clearfix select_dropdown mg_t15">
            <div class="pull-left">
                <label class="control-label pull-left f12 lineH20" for="input01"><em class="col_fb2727 mg_r5">*</em>库房编号：</label>
    ...
    watch: {
        ...
        show(val){
          if(val && this.model.CityId){
            this.province=[this.model.ProvinceId.toString()];
            this.city=[this.model.CityId.toString()];
            this.area=[this.model.ParentId.toString()];
          }
        }
    },
    computed: {
        valid() {
            if (!this.province.length || !this.city.length || !this.area.length || !this.model.HouseName || !this.model.HouseCode) {
                return false;
            }
            return true;
        }
